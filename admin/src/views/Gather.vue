  ...
  <div>
    ...
    <el-card>
      <template #header>
        <div class="head">
          <span>列表入口</span>
          <el-button type="primary" size="small" @click="openProduct()">新增入口</el-button>
        </div>
      </template>
      <p class="intro">封面、店名、套餐价在「宴会专题」统一配置；这里只决定出现在哪个分类/地区。</p>
      <el-table :data="products" stripe>
        ...
        <el-table-column label="专题门店" min-width="220" show-overflow-tooltip>
          <template #default="{ row }">{{ row.title || storeLabel(row.detail_id) || '—' }}</template>
        </el-table-column>
        ...
        <el-table-column prop="tag" label="角标" width="100" show-overflow-tooltip />
    ...
    <el-dialog v-model="productVisible" :title="productEditing ? '编辑入口' : '新增入口'" width="560px">
      <el-form label-width="100px">
        <el-form-item label="关联门店" required>
          <el-select
            v-model="productForm.detail_id"
            filterable
            placeholder="选择专题门店（封面/店名/套餐价来自这里）"
            style="width: 100%"
          >
            <el-option
              v-for="s in linkedStoreOptions"
              :key="s.id"
              :label="s.label"
              :value="s.id"
            />
          </el-select>
          <div class="hint block">请先在「宴会专题」配置门店封面、店名和套餐；这里只做去哪聚列表入口。</div>
        </el-form-item>
        ...
        <el-form-item label="角标文案">
          <el-input v-model="productForm.tag" placeholder="可选；留空则用专题角标" />
        </el-form-item>
...
const linkedStoreOptions = computed(() => {
  const options = []
  const seen = new Set()
  for (const n of nyeStores.value) {
    if (n.enabled === false) continue
    const sid = (n.store_id || n.id || '').trim()
    if (!sid || seen.has(sid)) continue
    seen.add(sid)
    const home = stores.value.find((s) => s.id === sid)
    options.push({
      id: sid,
      label: n.name || home?.name || sid
    })
  }
  return options
})
...
function storeLabel(id) {
  if (!id) return ''
  const hit = linkedStoreOptions.value.find((s) => s.id === id)
  return hit?.label || id
}
...
async function saveProduct() {
  return run('save-product', async () => {
    ...
    const linked = nyeStores.value.some(
      (n) => n.enabled !== false && (n.store_id || n.id) === productForm.detail_id
    )
    if (!linked) {
      ElMessage.warning('这家门店还没有宴会专题，请先在「宴会专题」配置')
      return
    }
    const payload = {
...
async function removeProduct(row) {
  return run('product-' + row.id, async () => {
    await ElMessageBox.confirm(`确认删除入口「${row.title || row.id}」？`, '提示')
